...
def get_DTU_data(name="S1", timelen=1, data_document_path="/root/autodl-tmp/DTU"):
    ...
    train_loader = DataLoader(dataset=CustomDatasets(train_data, train_label),
                              batch_size=args.batch_size, drop_last=True, pin_memory=True, num_workers=4)
...
def get_KUL_data(name="S1", time_len=1, data_document_path="/root/autodl-tmp/KUL"):
    class CustomDatasets(Dataset):
        # initialization: data and label
        def __init__(self, data, label):
            self.data = torch.Tensor(data)
    ...
    def read_prepared_data(args):
        ...
        for l in range(len(args.ConType)):
            label = pd.read_csv(args.data_document_path + "/label/" + args.name + args.ConType[l] + ".csv")

            for k in range(args.trail_number):
                num = args.name[1:]
                filename = args.data_document_path + "/" + str(num) + "/" + args.name + "Tra" + str(k + 1) + ".csv"
