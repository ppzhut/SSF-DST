...
from model_module import SSF_DST
...
def main(name="S1", time_len=1, dataset="DTU", result_queue=None):
    ...
    if result_queue is not None:
        result_queue.put((loss, acc.item()))
    
    return loss, acc

if __name__ == "__main__":
    file_handler = logging.FileHandler(os.path.join(current_path, 'log/result.log'))
    ...
    multiple = 1
    process = []
    names = ['S' + str(i+1) for i in range(17, 18)]
    
    # 使用Queue来收集每个进程的结果
    result_queue = Queue()
    
    for name in names:
        p = Process(target=main, args=(name, config['time_len'], dataset, result_queue))
        p.start()
        process.append(p)
